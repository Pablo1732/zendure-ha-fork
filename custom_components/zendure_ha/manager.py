        ...
        for d in self.devices:
            if await d.power_get():
                ...
                # SOCEMPTY means, it could not discharge the battery, but it is still possible to feed into the home using solarpower or offGrid
                    ...
                    # Off-grid idle device with battery disabled: add solar to outputLimit
                    # so solar flows to the load instead of charging the battery.
                    if d.pwr_offgrid > 0 and not d.batteryEnabled:
                        await d.solar_passthrough()
